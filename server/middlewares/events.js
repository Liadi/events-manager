module.exports = {
  validateEventFields(req, res, next) {
    if (req.method === 'GET' && req.query.eventTime) {
      validTime(req.query.eventTime, req, res, true);
    }

    // validate time
    if ((req.method === 'PUT' && req.eventTime) || (req.method === 'POST')){
      if (!validTime(req.eventTime, req, res, false)) {
        return 1;
      }
    }
...
function validTime(time, req, res, search) {
  if (!time) {
    if (!search) {
      res.status(400).json({
